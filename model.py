...
import random
from patches import MultiscalePatchExtractor, MultiscaleImageReconstructor
...
class MultiscaleVisionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.reconstructor = MultiscalePatchReconstructor(img_channels,
                                                          num_channels,
                                                          max_size,
                                                          patch_size,
                                                          num_levels)

        # frontend_feat_scale scales frontend's output features to give
        # a term added to the encoder's input features.
        self.frontend_feat_scale = nn.Parameter(num_channels)
        ...
        self.frontend = TransformerEncoder(encoder_layer=encoder_layer, num_layers=frontend_layers)

        self.encoder = TransformerEncoder(encoder_layer=encoder_layer, num_layers=encoder_layers)
        ...
        self.encoder_subset_proj = nn.Linear(num_channels,
        self.encoder_subset = Subset(num_channels)

        self.decoder_subset = Subset(num_channels)
    ...
    def forward(self,
     ...
     decoder_patches: the number of patches to select for use in the decoder
        ...
        input_emb = self.extractor(x_and_mask, num_levels=1)
...
class PatchExtractor(nn.Module):
    ...
    def forward_indexes(self,
       ...
       h_offset, w_offset: height and weight offsets, each of shape (N,) i.e. batch size,
        ...
        emb = torch.gather(emb, dim=1, indexes=rel_indexes[..., None].expand(*rel_indexes.shape, emb.shape[-1]))
        emb = emb * mask[..., None]
        return emb, mask
...
class MultiscalePatchExtractor(nn.Module):
    ...
    def forward_indexes(self,
        ...
        for level in range(self.num_levels):
            s = self.subsample_factors[level]
            embed, mask = self.extractors[level].forward_indexes(
                _subsample(x, s), indexes, _div(h_offset, s), _div(w_offset, s2))
            embs.append(embed)
            masks.append(mask)
        ...
        emb = torch.stack(emb, dim=0)  # embs: (num_levels, batch_size, num_embed, num_channels)
        emb = (emb * index_offset_mask.to(torch.float)[..., None]).sum(dim=0)

        if __name__ == main or random.random() < 0.01: # TODO: could remove this
                                                       # random part eventually
            assert index_offset_mask.to(torch.long).sum().item() == indexes.numel()
            mask = torch.stack(mask, dim=0)  # (num_levels, batch_size, num_embed)
            assert torch.allclose(mask, index_offset_mask.to(torch.float))
        return emb
